...
**Why.** Her words on the first `lab-0924` pass: *"wayy too blurry the background and the text sometimes is cut by the thumbnail up and this pic is horrible."* The blur note is the second in three days. The 0.035 safe clamp had passed lines that her feed cut, so the clamp is a floor, not air. The frame was chosen because PREFS said it had replaced a flat one; she prefers the flat one.
**What changed.** `PREFS.md` (her words, dated), `assets/thumb-templates.json` (`top_min`, the line builder), `scripts/thumb-check.py` (`TOP`), all 21 cards rebuilt with her lower in the frame at 0.38 so the line clears her head, `/thumb-lab` republished.
...
### 2026-09-24  all three reels post today, the sitting guard overridden on her say-so
**Decision.** Post all three release-week reels today by hand, 45 minutes apart, overriding the same-sitting guard.
**Why.** She asked a second time, plainly: "can you post it today tho". A concern raised once and reaffirmed is her decision. Release week is the whole point of these and the guard costs two days of it. The override is narrow: `--post-next --id` is a supported route that still applies the footage rest rule and the already-said check, so only the sitting rule is set aside, and the 45 minute spacing is the gap her own queue configures between Instagram posts.
...
**Decision.** Let the same-sitting guard stand rather than override it, so one of the three new reels posts today and the others follow on the next days. Offered her the manual route for the other two instead.
**Why.** All three come from one sitting (one shirt, one room, one hour), and the queue refuses a second reel from a sitting Instagram has shown in the last twelve posts. That guard exists because she twice reported her reels all looking the same, and there is no supported override for it at post time. Defeating a guard she asked for, silently, to hit a same-day count is the wrong trade.
**What changed.** New reels moved to the front of the reel lane; copies of all three plus captions in `~/Desktop/mirror-reels-today/`. Her call was put on the desk and she answered it within the hour.
**Status.** superseded by the 2026-09-24 entry above, on her instruction
...
**Evidence.** Re-run offline on the same jealousy plan: 19 trims instead of 38, "three" back at raw 35.30, her 1:46 join still at 0.15 s of air. Commit "perfect-joins: a short word is not a click" on origin/main. The held file: `WATCH-THESE/_FAILED - types-of-jealousy.mp4` at 10:00.
**Status.** live. The honest answer to her question is in this session's reply: the gates measured a narrower fault than she judges, fixes went into single takes instead of the cutter until 23 Sep, the picture was never checked, and the loop ran through her ear a day late.

### 2026-09-24  the re-cut batch gets its thumbnails built to the templates, two per video, before she picks
**Decision.** Every video in the re-cut batch has two candidate cards built to its template from a same-session still, gated by `thumb-check.py`, and put on `/thumb-lab` for her pick; nothing is set on YouTube until she picks.
**Why.** Her ask: build the proposed improvements with full creative control, per kind of video. Nine of the eleven had no finished card or one that failed the gate, and two had nothing built at all. Full creative control still runs inside her settled rules: Times, lowercase, her face centred, the light room, the flag on ace videos, the note on long-forms. The lines were written in the four shapes the audit ranked and the label and the mood were not used.
...
**Thrown away.** The wrap card for the ace books: the display word ran behind her head and died at 210px. Replaced by the line template with Loveless propped and the flag on.
**Not acted on.** The picks, and the YouTube swaps that follow them. Two outfit calls are hers on am i anxious.
**Evidence.** `/thumb-lab#lab-0924`, the checker JSON under the session scratchpad, commit on origin/main.
**Status.** live, waiting on her picks.

### 2026-09-23  thumbnails get one template per kind of video and a gate that measures the finished card
**Decision.** Every thumbnail is built to one of seven named templates whose bands live in `assets/thumb-templates.json`, and `scripts/thumb-check.py --template <name>` measures the finished pixels and refuses a card before it is shown to her. The line is written in one of four shapes, all four drafted before one is chosen, and a line that repeats the title's words is struck.
**Why.** Her ask, in her words: audit how the thumbnails are done and make a system so "the size of the text on the thumbnail is big enough, it's centred enough, and we always have templates for each kind of thumbnail". The 90-day window to 11 September (39 long videos, 5,000+ impressions) says type height is the strongest lever she controls (above-median 3.98% against 3.06%), every extra text block costs (r = -0.31), readable small print medians a point lower, and face size and grade move nothing. The Monday audit had already found 9 of 15 scheduled cards with the last word under the runtime badge, and nothing mechanical was catching it. 78 winning cards from ten other niches confirm the direction (tall stacked box, one accent word, the number as the object, a mid-expression face) and rule out copying their three-block caps-sans banners, because her own numbers say one block.
**What changed.** New `scripts/thumb-check.py` and `assets/thumb-templates.json`; `build-thumbnail` SKILL.md runs the gate in step 5 and its checklist; `/thumbnails` opens on the audit (the 39 measured, the 78 from other niches, the four line shapes ranked, the template table, the re-cut batch card by card); desk item on the youtube branch. Nothing in PREFS.md moved, because that file only records what she has said.
**Not acted on yet.** The eleven re-cut cards: nine fail the gate and two are unbuilt. Rebuilding needs the drive with the source stills plugged in, and her read of the proposed lines first.
**Honest limit.** The gate flagged 7 of her top 12 as well as 6 of her bottom 12. It is a floor for what she reads as ugly, not a predictor of a click. The claim in the line still decides the rate.
...
**Why.** The GPU gate reports **0 faults across 169 joins** on the new plan. That is the same gate, on the same take, that found the QPR clip on the old cut when the CPU gate passed it, so a clean verdict from it means something a clean CPU verdict would not have. The word now starts at raw 708.3, exactly on the Q's onset, where the old blade cut at 707.79 and took the Q with the dead air. And the new plan carries four `mid-phrase-restore` spans, so the caption fix written earlier today returns **28 words** to the captions on the next render: "a lot of our friendships and romances have", "know what to do or you do", "had to i literally", "i want you to take from this video and". Those are the places she currently speaks with nothing on screen.
**What is left, and why it is worth another pass.** Two abandoned attempts (1.10 s at 2:14, 0.61 s at 1:52, which is the "fully" spot) and two gaps inside a word (1.00 s at 4:11, 0.69 s at 8:15). All four are the kind she hears. 4 cut ranges and 11 retimes are written, zero overlaps. The per-join cache was populated by this run, so the next pass re-proves only the joins that move.
**Evidence.** `~/Movies/riadms-work/proof/alt-NEW/alterous-attraction.proof.listen-cut.md`.
**Status.** live, one pass to go.

### 2026-09-23  she heard a pause she was not looking at the camera through, and that is a new class
**Decision.** `scripts/look-away.py` is the first check on the picture. A pause she holds the camera through is a beat and stays; the same pause with her eyes down is a hole.
**Why.** Her report on types-of-jealousy: "1:46 there's a moment there of break and pause in which I'm not even looking at the camera." The pause is 0.56 s, under the 0.60 s the ear check flags at, and every audio check passed it. Its length was never what was wrong with it. Apple Vision and `vision/facekit.py` were already on the Mac, so the check is frames sampled out of each pause against the take's own baseline: her open eye measures 0.287, it reads 0.131 at 106.20, her eyes are down 0.20 s, and it flags 1:46.20 while clearing the two pauses either side. It runs on the raw take, so it answers before a render exists, at about a minute a take.
**And the fault underneath was the old one.** "read" is recorded running to 314.48 when her voice stops at 313.32, inflated by 1.12 s, and "into" starts 0.31 s before its record. Both retimed.
...
**Decision.** the-deadline-nobody-set is delivered as `_NOT-CHECKED` and ready for her CapCut pass. It is the first take to go from a fault she heard to a clean file with the sound proved before the render rather than after it.
**Why.** Her report at 0:27, "two probably's and a small weird pause between them". The proof of the old plan reproduced it (1.38 s inside "you'll"); one retime and one cut were written; the proof of the re-cut's plan showed it gone, three seconds after the plan existed and an hour before the render finished. Then the render: drift-check in sync, cut-integrity 0 faults on 28 pieces, the listening gate 27 joins 0 faults, dead-air on the delivered film 0 silences of 0.6 s or more, and two frames at the spot show the caption "say i figured" with her mid-word. What she would hear and what she would see, both checked.
...
**Status.** live. Her CapCut pass is the next step, and the 16 September `the deadline.mov` on her Desktop is of the old cut.

### 2026-09-23  the listening gate on the GPU: measured, and the difference was the GPU being right
**Decision.** Not acted on. The gate stays on the CPU by default, because the standing instruction for the experiment was "any difference means it stays on the CPU and you write down what differed", and there was a difference. The call to move it is hers, with a recommendation to move it.
...
**Status.** decided and not acted on, waiting for her.
...
**Why.** She asked for fast vertical vlogs with text, sound effects and a viral sound, and nothing here made one: the `vlog` skill is landscape long-form, `vertical-short` is talking-head, `edit-video-dense` is talking-head. The length was the only real decision. The 15 vertical vlog Shorts measured for this median 38.9s, and building to that would have rebuilt the lane her own numbers retired on 2026-09-15: 233 of her videos at 60s or under produced 37 net subscribers between them, 0.10 per 1k. Ranked by punch (views over subscribers, so channel size is not doing the work) the pool says the same thing: 61-120s medians 0.35 against 0.20 for 30s and under, and the two best vlog Shorts in it are 94.9s and 108.4s. 100 seconds sits inside the 75-150s mini band CLAUDE.md rule 17 already asks for, so the format agrees with her strategy instead of arguing with it.
...
**Evidence.** `creator-research/VLOG-SHORTS.md`; `creator-research/anatomy-shorts-vlog.md` (15 Shorts, Apple Vision, 2 frames a second); `creator-research/shorts-vlog/TOP.md` (348 Shorts from 15 channels). The cover-vs-contain frame comparison was rendered and looked at. **The end-to-end render on her real footage is UNVERIFIED: the Toshiba holds the DJI scenes and it is unplugged.**
...
**Why.** `shorts-refs.mjs` asks YouTube whether a video is a Short with a HEAD request, 200 for yes and 303 for no. From an unconsented IP YouTube 302s every request to its consent page before it looks at the video, so the answer was no for real Shorts and long-form alike. A vlog sweep found 231 candidates and confirmed 0, with no error anywhere. Checked against her own published Shorts, which are known-good: they returned 302 as well. This is the always-green failure LEARNINGS.md keeps recording, and left alone the next `--refresh` on the essay pool would have replaced 242 confirmed Shorts with an empty list.
**What changed.** `shorts-refs.mjs` sends a consent cookie and counts checks that answer neither 200 nor 303; an empty sweep now throws. Two additions beside it rather than on top of it: `--set <name>` namespaces a sweep so the vlog question could not overwrite the essay pool, and `--queries <file>` supplies a different query set. New `scripts/shorts-sweep.mjs` asks the same question with yt-dlp and no API quota at all, after 18 queries exhausted the day's 10,000 units, which is the same quota her uploads spend at 1,600 each.
**Evidence.** `scripts/shorts-refs.mjs`, `scripts/shorts-sweep.mjs`, `creator-research/VLOG-SHORTS.md` final section.
**Status.** live.

### 2026-09-23  YouTube search does not return Shorts, so read the /shorts tab
**Decision.** Sweep creators' own `/shorts` tabs rather than searching for Shorts by keyword.
**Why.** Eighteen queries with `#shorts` in them returned 60 videos under three minutes and the platform correctly confirmed that none of them were Shorts. YouTube's web search ranks long-form and its Shorts live in a separate shelf that `ytsearch` never reaches. It is also the better question: how a format is built is answered by reading the people who build it, not by a keyword that mostly finds whoever bought the phrase.
**What changed.** `shorts-sweep.mjs` reads `creator-research/queries/channels-<set>.json`. 348 Shorts from 15 channels on the first run against 0 from the keyword sweep.
**Evidence.** `creator-research/shorts-vlog/TOP.md`.
**Status.** live.
...
**Why.** She asked for at least ten videos fully edited in eight hours with something always running. The runner capped at two heavy stages, on a 1h27m measurement taken from a SHORT take, and the three renders already in flight when it started were the three longest in the set (43.4, 40.9 and 23.0 minutes of raw). The work scales with the number of joins, so a 43-minute take is not 2.5x the cost of a short one, it is many times it. Not one of the twelve queued takes ever started all night while more than half the machine sat idle. One video was delivered. Stopping the competing lane drivers more than doubled the CPU reaching real work, 102% to 223%.
...
### 2026-09-23  "is the writing style of what you wrote my writing style?" No. So a system that writes next to her sentences
**Decision.** Build a voice system that measures her WORDS and SHAPES per narrator, from the live draft with every paragraph written in this repo removed, and gates new prose against the band her own chapters score in. The tools: `scripts/lib/her.mjs` (the corpus), `scripts/her-lexicon.mjs` (build, report, `--check`), `scripts/her-nearest.mjs` (her closest passages to a scene, by move, by speaker), `scripts/her-brief.mjs` (one command before writing), and the `write-as-her` skill. `her-voice.mjs` now reads the same corpus.
**Why.** She asked whether the Draft 6 and Draft 7 prose was in her style. `her-voice` said the rhythm was in range and it was, and it still was not hers: "and it is the first thing I have ever kept from Dita" at 8.3 per 10k against her 1.8, somebody/nobody at 21.5 against 6.9, eleven "I'd" in 4,864 words where she uses none, fragments at 23 percent against her 16, and none of her own recurring constructions present. Two faults underneath: `her-voice` had been built off a pull of the RETIRED `doc (draft).docx` from 11 September (its top constructions were "flyer girl" and "r o"), and DRAFT 6 carries 4,855 of my words that would otherwise be measured as hers. Retrieval matters more than any number: the brief puts her five nearest passages on screen before a sentence is written.
**What changed.** Four new scripts and a skill; `her-voice.mjs` reads `herCorpus()`; `assets/her-lexicon.json` and `assets/her-voice.json` rebuilt from DRAFT-6 minus 207 of my paragraphs. Calibration, leave-one-out over her 51 chapters: coverage median 95.4%, my tics p90 40.7 per 10k. My Draft 6 prose fails that gate at 45.2; a rewrite of the Draft 7 interview passage done beside her Chapter Fifteen passes at 0. Her Chapter One sits at exactly the p90, so the gate is a floor and not a bar.
**Evidence.** `node scripts/her-lexicon.mjs --report --pov yana`; `node scripts/her-brief.mjs --pov yana --scene "..."`; `.claude/skills/write-as-her/SKILL.md`; the desk.
**Status.** live. The blue in both change-docs is still the old prose; rewriting it through this loop is the next thing, on her yes.
...
slots lands between 18:30 and 07:45 Berlin. On the YouTube side her own numbers
...
subscribers out of 219 videos, so the volume she asked for is built at the length
that converts. She can overrule it and the job takes a flag.
...
goes to 15 only when she films longer clips. Not decided by preference: it was
...
straight copy is a long card. Of her 277 face clips, 21 run 15 seconds or more,
...
**Why.** She read `/filming` and said she was fairly sure *five more types of
attraction* was already filmed. She was right. It was shot 24 August, 23:03 raw,
...
**Status.** live. Two questions are open and only she can answer them: the
...
not queued.
**Why.** She asked for the strategy step by step and for scripts. The schedule
...
it fits the sahar shape without borrowing her words: the image is ria's own
August face card. The Meana "being desired" claim was kept out because it could
only be traced to interviews, not a citation, and she says sources out loud as
fact. The face cards are built from strangers' real comments under
`face-cards-07-26` because five of her six top Instagram posts are that shape.
...
adjacent channels plus her own last 90 days, published as `FILMING-PLAN.md` and
...
**Why.** She asked what the wider love and personal development essay lane is
...
almost exactly hers, and her last fourteen long-forms median 10,571 views, or
1.10x her subscriber count, against 0.40x for the next best channel measured;
her titles are proverbs, which contradicts the search-shaped title rule in our
...
search-shaped title was never the engine. Third, her reach and her conversion
...
through, so the competitor set is eleven channels chosen from her own research
...
**Why.** Her standing rule, 2026-09-22: *"write the article always"*. Reading the cut of every long-form that is not already public found 25 places where she sends a viewer to the site, and three had nothing behind them. A promise with no page is not a line to cut from the video, it is a page to write, and it has to exist before that video gets a date.
**What each page adds that the video did not.** The likes-you page splits the 28 per cent back into 36 per cent for men and 18 for women, which every write-up averages away, and adds Montoya, Kershaw and Prosser's meta-analysis of 286 studies finding that the signs really are correlates of attraction. That makes her argument sharper rather than softer: the list is true about a population and unreadable at one table. It also adds the 2022 replication where third-party observers judging speed daters landed at chance. The gender envy page adds the benign-against-malicious envy distinction, which reframes the sting she describes as the kind of envy that points at becoming, and says out loud that gender envy is community vocabulary with no paper defining it while the nearest measured thing is gender euphoria.
...
**What changed.** `scripts/place-overlays.mjs` follows a re-filed clip and says so in the log. `scripts/relocate-beats.mjs` reads the lanes off the disk, and where more than one holds a cut of the same take they have to agree word for word, because a beat is a time in one cut and the wrong one plays the wrong half-second of her talking.
**Evidence.** commit `6670da52`, guard case 9, 16 passed and 0 failed.
**Status.** live

### 2026-09-22  the capstone documents rewritten so they do not read as machine-written, and design taken out of the LO arithmetic
**Decision.** Rewrite the Project Brief, the folder README and the Drive START HERE page in plain prose, and rebuild the LO section without a design major in it.
**Why.** Two things from her. First: "make the docs not look like they are ai make them look normal and notes and etcetc". The brief passed the detector at 2 of 100 and still read as machine-written to a person, because the tells it cannot see are formatting ones: 117 bold spans, a thesis sentence opening nearly every paragraph, 44 table rows where prose would do, and the same antithesis shape over and over. Second: she is not a design major or minor for this assignment, and the LO section was built as though Arts and Humanities were a second major sending two LOs.
**What changed.** Bold is now zero in both documents, the backticks are gone so prose is not set in a code face, the two tables that hold real data survived and the rest became sentences, and the rewrite is in the first person because a brief is written by a student and not about one. 4,908 words down to 4,243. `ai-tells.mjs` reads 0 of 100, Clean, against her published-article bar of 0 to 2.
**The LO correction, which is substantive and not cosmetic.** The handbook requires the 4 Capstone LOs plus at least 5 from **any** of the colleges, and separately at least 2 from each major held. So her AH113 outcomes are still usable, because they never needed to come from a major; they only needed to exist and be evidenced. The section now reads: 4 from Computational Sciences, which satisfies the two-from-my-major rule under any reading of an independent track, 2 from courses taken in other colleges, stated as not a second major, and 3 custom LOs, which the handbook says count toward the same total. Nine against a band of five to ten.
**What this does to the custom LOs.** They carry more weight here than they would for somebody with two declared majors, because `#silentfailure`, `#audienceinstrumentation` and `#emotionalsafety` are now where the design and systems side of the work is assessed rather than borrowed from a major she does not have. That is an argument for proposing them, and it is written into the brief and into the committee questions.
**What was thrown away.** The first rewrite of START HERE, twice: once for em dashes, once for the design-coursework line. Both replacements were read back out of Drive rather than assumed.
**Evidence.** `capstone/Project-Brief-CP193-ria-dumitrescu.pdf`, now 9 pages rather than 12, pages read as images. The Drive folder refreshed.
**Status.** live
...
**Why.** She said three things this session that the documents had wrong. The advisor was not named anywhere. The plan of 11 September asserted a dual major in Design Across the Humanities and Data Science and Statistics, which she has never had, and the whole two-LOs-per-major arithmetic was built on it. And Prof Tam has already agreed to the one-venture framing, which the brief was still arguing as a proposal and hedging around.
...
**What is still blocked and on her.** The Project Brief Cover Sheet is behind a Minerva login and returns 401 from here, so the table rows cannot be filled in. She copies it, pastes sections 1 to 9, and exports the one PDF.
...
**Decision.** `onEitherDrive` in `scripts/lib/paths.mjs` now resolves against `~/Movies/riadms-work/offline` as well as the two drives, last in the list, and the raws for the takes in flight were copied there before she unplugged the LaCie.
**Why.** She asked how long until she could take the drive, and the honest answer was hours. The cut stage works off audio already cached on the laptop, so nothing had the drive open at that moment, but the render step reads the raw video back, and those two raws are 4.1 GB and 9.6 GB and existed only on the drive. Unplugging would have failed both renders and everything after them.
...
**The limit, stated rather than hidden.** With the drive out, `edit-queue.sh` still copies the finished video, the overlays and the keep directory back to `$LF/<take>/` on the drive, and those copies now fail. There is no `set -e`, so the run continues and the video still lands in `WATCH-THESE`, which is the folder she opens. The cost is the reusable cache: a later re-render of those two takes would redo the whole 90-minute cut. The fix is one line in the delivery step and it waits, because zsh reads a running script incrementally and editing it would corrupt the run.
...
### 2026-09-22  the article a video promises gets written, and the food b-roll comes out
**Decision.** Her rule, in her words: "write the article always". A line in a video promising a page on riadms.com is now a thing to build, never a line to cut. The a little life article is live at /guide/a-little-life-friendship. And alterous loses its food b-roll: she watched the finished render and said food b-roll makes no sense here.
**Why.** a little life ends on "I did write a bigger article about this on my website" and no such page existed, so the video was about to ship a promise to its viewers that the site would not keep. On the b-roll: the clips were hand-picked in August as "friendship-coded: shared food, shared tables", and food illustrates nothing she says.
**What changed.** New chapter in Part IV of the field guide with twelve opened and checked sources (Greenwell, Mendelsohn, Brotto & Yule 2017, Borg & Thai 2025, Gewirtz-Meydan & Opuda, AVEN, Resources for Ace Survivors, support lines) and four concept cards. Two clips swapped in the alterous spec, one refused. Three faults found in the editor while doing it: a spec clip the spec no longer names was never removed from the b-roll list so both played on the same second (place-overlays), a killed run left stamps that the next run read as fresh (edit-queue), and the delivered-stale check hashed the drive's decisions file while the cut stage stamps the repo's (edit-queue).
**Evidence.** Commits a391e569 (article), 21e13c68, c6f7a27b, d9cf6f8a. ai-tells 1/100 HUMAN_ONLY, burstiness 0.89, no em dashes. alterous: 0 faults in 170 joins, sync +4 ms.
**Status.** live

### 2026-09-21  the join fixer went onto the GPU at her yes, and back off three hours later
**Decision.** The fixer (perfect-joins) ran on the GPU from 19:50 and was put back on the CPU at 21:30. The gate never moved.
**Why.** She said yes to the split proposed below (fixer on the GPU, gate on the CPU). On its first real run it cut a little life in 20 minutes instead of 2 hours, and the CPU gate then held it for 5 joins, three of them exactly where the GPU had chosen a different edge from the CPU in the afternoon comparison (1:06, 1:27, 2:09). The CPU-fixed cut of the same take had none left once the checker was corrected. A held video costs a full extra round, so a fixer that is six times faster and wrong more often is slower end to end. The proposal assumed the gate catching misses made them free; it does not.
...
**Why.** Her ask: audit it against the business and the channel and make it "an actual
...
a brooding screen at onboarding, which should be designed with her rather than written
...
**Status.** built on a branch, waiting on her phone and her word to merge.
...
delivered the file anyway. She watched it and called it weird for two days while
...
### 2026-09-19  the picture was running ahead of her voice on every video
...
**Why.** She called the alterous re-cut "weird" for two days while every audio
...
at the end. Her lips were out of sync the whole video and worsening, and her
...
**Evidence.** Verified on her own footage before applying: five real spans, 353
...
### 2026-09-19  the power is parts of a person, and every part has its own price
**Decision.** Her design, built live. There are parts of a person (thought, feeling,
memory, the body, and what a person is made of, which is never named) and two things
she can do to each: **hear it**, which leaves it in them, or **take it**, which
removes it permanently. Hearing costs her something different per part. Taking always
costs the same: whatever she takes, she becomes, and she cannot give it back. Pain is
the only cost that heals, which is what makes it the tempting one.
**Why.** Her words: "i think i want the power to be like connected to different parts
of humanity... like different costs for different things." My own four versions and
my earlier "the full cannot feel" rule were both rejected, correctly: they were
labels rather than mechanisms, and she said so twice.
**What changed.** `HER-POWER.md` is new and supersedes the four rungs in
`THE-GRIEF-ENGINE.md`, the four versions in `HER-POWER-FOUR-VERSIONS.md` and the
mechanism in `THE-TOUCH-RULE.md`. It also explains Sanda's notebook for the first
time: she was paying in memory and writing herself down before it went, and Ivy's
shoebox is the same habit with nobody to tell her why.
**Thrown away.** My rule that *the full cannot feel*, and all four of the versions
I wrote for her to choose between. She wanted a system, not a switch.
**Evidence.** `HER-POWER.md`, questions 280 to 284.
**Status.** live.
...
see either fault.
**Why.** She listened to the 11:53 alterous re-cut and said it was still weird. The
night's work had measurably fixed what I thought she meant: the cut at 0:57 moved
from -36.3 dB to -53.9 dB and truncated word-decays went from 136 to 7. She was
right and the instruments were wrong. What she was actually hearing at 0:56-0:57
was one two-second piece made of three separate faults: the dying tail of a word
whose beginning had been cut away, 1.43 seconds of silence at -80 dB, and her
"and" chopped off mid-word - and beyond it, a cut that had removed "a lot of our
friendships and romances have", the subject and verb of her sentence. She had
described all three in her own words days earlier ("weird sounds happening before
...
verified on a plane. On alterous the rules restore her words at four places and
...
worst spots, each as it was and as it is now, for her to judge by ear.
**Status.** live
...
entries for three files that no longer existed. Three cards carrying her room were
...
already carrying a sound she chose, all seven at posting level.
**Status.** live
...
total` and proceeds.
**Status.** live. **Open for her:** rain sharing the Instagram bucket is still
...
**Decision.** Rebuild the word reel on her Escapism clip at 6 words, ingest the whole 15 September sitting into `reels-content/dance/`, and make the overlay band a per-sitting spec option. Supersedes the footage-constraint finding in the entry below, which was wrong.
**Why.** She said: *"but I gave you like more dancing videos of me, like maybe 10 of them."* She was right. The scan behind the earlier claim only covered `reels-content/`, the **ingested** library. The sitting she airdropped on 15 September had never been ingested, so it sat in `~/Downloads/video/` under hex names where no catalogue could see it. **Six of those clips are 11 to 19 seconds**: Escapism 18.9, bloodstream stripped 15.1, UNDER THE STARS 15.1, Love The Way You Lie 15.1, Mercy 15.0, Training Season 11.1. The desk had already recorded that this sitting existed and had never recorded how long the clips were, which is why a note about framing got read as a note about length.
...
**The band was set by looking, and the measurement would have got it wrong.** The blue satin sitting frames her head at about 21 per cent down rather than 45, so the block moved to y 64-294. A row-variance scan of the source claimed her hands crossed the text in 12 per cent of frames; pulling the worst frame showed her hands nowhere near it, because the detector was reading the static ceiling line and the smoke alarm as content. Rule 1 again: the automatic check was confidently wrong and the frame was right.
...
### 2026-09-18  nuuko gets an Android app, and the local-first app turns out not to work offline
**Decision.** Ship Nuuko as an Android app built with Capacitor 8 around the same `public/` the website serves, with the daily moment handed to Android's own scheduler; and fix the service worker first, because the offline story the app is sold on was not true.
**Why.** Her ask was "can you build it as an android app too". Wrapping a website earns nothing on its own and Google Play rejects one on minimum functionality, so the app had to be worth installing. The thing that makes it worth installing is the reminder: `notificationService.js` schedules with `setTimeout` and delivers with `new Notification(...)`, and both die when the page does. The Notification constructor does not exist in the Android WebView at all. So the daily moment has only ever fired when a tab happened to be open, which is the same as never, and a journal whose reminder cannot reach a closed app is a journal nobody returns to.
**What was found on the way, and it is the bigger finding.** The app calls itself local-first and had **no offline cache at all**. `CORE_ASSETS` in `public/service-worker.js` listed `/pwaInstall.js` and `/capacitorBridge.js`; neither file exists, and both 404 on www.nuuko.app today. `cache.addAll()` rejects the whole batch if one request fails, so the static cache was empty on every install and the install event itself never resolved. Separately, every clean URL the site actually uses (`/write`, `/people`, `/home`) fell past every branch of the fetch handler with no `respondWith`, because the handler only matched `.html` and `/`. Measured, not assumed: against the old worker the static cache holds **0 entries**, the worker never reaches `activated`, and `/people`, `/write` and `/home` all fail with `ERR_INTERNET_DISCONNECTED`. Against the new one the cache holds 28 entries and all four pages open offline.
**What changed.** In `nuuko-v1`, branch `capstone/android-app`. The worker caches one asset at a time so a single missing file costs one file and prints which; a navigation handler maps clean URLs to their page and falls back to cache; `people.html` registers the worker at all, which it never did. `capacitor.config.json`, a committed `android/` project, `public/capacitorBridge.js` (native, a no-op in a browser), `public/scripts/apiBase.js` (`/api` is relative on the web and absolute in the app, or every account call fails against `https://localhost`), `scripts/build-android-www.cjs`, `scripts/smoke-offline.cjs`, `android/SIGNING.md`, `about-nuuko/the-android-app.md`.
**The bundle is not `public/`.** `public/` is 193 MB: 64 MB of ONNX emotion model, 31 MB of Instagram marketing, about 40 MB of `.original` files the image optimiser left behind. That is over Play's 200 MB base-APK limit and absurd for a journal. The build copies an allowlist and **exits non-zero over 60 MB**; the result is 33 MB and the APK is 38 MB. `index.html` is marketing and is replaced in the bundle by a shim to the shelf, because somebody who just installed the app does not need to be sold it.
**The privacy decision inside it.** `android:allowBackup` defaults to **true**, and true means Android copies the WebView's IndexedDB and localStorage, which is every entry, into the user's Google Drive, outside anything the app encrypted and outside anything `privacy.html` promises. Set to false, with device transfer excluded too. Read against `about-nuuko/the-law.md`.
**Not acted on, deliberately.** No keystore was created; it is the one file in the project that cannot be regenerated and it is hers to hold (`android/SIGNING.md`). Google sign-in does not work in a WebView and Google blocks it on purpose, so sync inside the app is not built; the app has no login wall so nothing else is blocked by it. `@capacitor/push-notifications` is installed and nothing calls it, which puts `c2dm.permission.RECEIVE` in the manifest that Play's data-safety form will ask about: remove the dependency or plan the feature.
**Evidence.** `node scripts/smoke-offline.cjs` passes on the branch and fails with 5 failures against `main`'s worker, both runs in this session. `node scripts/smoke.cjs` unchanged. APK at `android/app/build/outputs/apk/debug/app-debug.apk`, 38 MB, `minSdk 24`, `targetSdk 36`.
**Then it was run, and building it green had proved nothing.** Installed on a Pixel 7 emulator and driven by hand, which found six faults that every check passed: the APK opened on the marketing landing page (the entry shim was built but never synced before the build); the splash never came down, because the bridge hid it at the END of boot and welcome.html, the first screen a new install sees, did not load the bridge at all, so the app sat fully working underneath a splash forever; and the reminder was never scheduled three separate ways — `getTargetHour()` returns `{hour, source, reason}` and the object went into `setHours`, giving NaN, an Invalid Date, and a RangeError swallowed into the web fallback; Date objects serialise to **null** across Capacitor's raw bridge, so Android received `{"at":null}` and stored nothing; and `people.html`, the app's home screen, never loaded `notificationService` for the bridge to wrap. Also: the app asked for notification permission on first launch for a reminder that is off by default, which is the reliable way to be denied for good. Settings are read before permission now.
**SCHEDULE_EXACT_ALARM came in from the plugin's own manifest** and threw the user out into Settings > Alarms & reminders the first time a reminder was set. Google restricts that permission to clock and calendar apps, so a journal declaring it is a Play rejection waiting to happen. Removed at merge with `tools:node="remove"`; the plugin falls back to an inexact alarm and says so. `@capacitor/push-notifications` went with it. Six permissions left, all explicable on the data-safety form.
**And the one that would have bitten the first Play update.** `CACHE_VERSION` was a hand-typed string, so the service worker serves old JS after new JS ships. It happened during this build: a fixed `capacitorBridge.js` was installed and the app kept running the broken one, green build, nothing in any log. `scripts/stamp-sw.cjs` now derives the version from a hash of everything `CORE_ASSETS` names.
**What was seen, and what was not.** Seen: it installs, launches, opens on the shelf, runs the onboarding, files a person, opens the write page with its prompt and picker; the service worker controls the app; a notification fired and appeared correct — `nuuko`, `the moment is open.`, her green, the status-bar silhouette; the launcher icon masks properly. **Not seen, and it is the whole argument for the app: a reminder arriving with the app backgrounded did not reproduce on the emulator**, with or without `allowWhileIdle`. That is plausibly emulator Doze, and `force-stop` is not a fair test because Android refuses alarms to a force-stopped app — but it is unproven. Until it has been watched on a real phone the claim is an intent, not a result.
**Status.** live on the branch, not merged, not on Play. Needs her on a real device before main, and the background reminder is the thing to watch.

### 2026-09-18  measure her CapCut filter instead of guessing it, and find half of it cannot be automated
**Decision.** Rebuild the colour half of her CapCut filter inside the pipeline, measured off a matched pair of files rather than translated from the sliders, and keep the makeup half in CapCut because ffmpeg cannot do it. Her ask: *"find a wy for ht efilter to work perfectly, every long orm efore it s edited should have the filter."*
**Why.** She wanted pre-filtered raw files so Shorts cut out of a long-form would already carry her look. Two things came out of testing it. First, a second filtered copy of every raw is a large write to a 84 MB/s spinning disk, a lossy re-encode before editing, and a look that cannot be changed later without rebuilding the library. Second, and the one that actually decides it: **the makeup is most of what she sees, and it is face-tracked AR.** Blush, lipstick, eyelashes and face highlights are painted onto a per-frame facial mesh. Nothing in ffmpeg does that, so a pre-filtered raw would carry the 5% and miss the 95%.
**How it was measured, because the sliders are not ffmpeg's scales.** `the-lie-of-the-spark` exists as both the pipeline render (108.859s) and her CapCut export (108.867s), same timeline, so frames line up. Six frame pairs at 8/20/35/50/70/90s, sampled over four **background** boxes only — curtain, wall, window, desk — because her face carries the makeup and would have poisoned the colour measurement. 3.79M pixels. Her filter is R +8.15, G +2.37, B +0.27, luma +3.60, saturation x1.157: a warm-highlight lift that rolls off before the whites.
...
**One setting was rendered and rejected by looking.** `bilateral=sigmaS=10:sigmaR=0.14` for Smooth 54 erases her eyebrows and turns her hair to plastic. The light setting, sigmaS=6:sigmaR=0.08, evens her skin and keeps her. The strong one would have passed any numeric check.
**Not acted on, pending her verdict.** The look is NOT wired into the render path yet. She asked to see a 30 second test first and that is what exists.
**What changed.** `scripts/lib/capcut-look.mjs` holds the look and the provenance. Nothing else moved.
**HER VERDICT, and both halves of it were right.** *"the beautify is no where near stong enoghg and it s way too pink the entire video."*
**The beautify was weak because the wrong knob was turned.** `bilateral` has two: sigmaR is how DIFFERENT two pixels may be and still be averaged, sigmaS is how FAR APART. The first pass reached for sigmaR, which is intuitively "smooth harder" and is literally an instruction to smooth across edges - which is why it ate her eyebrows, and why it had to be run weak to be usable at all. Holding sigmaR low and raising sigmaS instead smooths hard inside flat skin and leaves every strong edge alone. sigmaS=60:sigmaR=0.05 is far stronger than the rejected setting and her brows, lashes, lip line and hair all survive. It does not flicker: frame-to-frame delta on a forehead patch is 4.89 filtered against 6.39 unfiltered, steadier than the source because it suppresses sensor noise. Costs 2.4x realtime, about 36 minutes on a 15 minute long-form.
**The pink is real and the measurement explains why she is right rather than wrong.** On her forehead her own export runs R-G of 34 against the pipeline's 25, so the warmth genuinely is hers and the match was accurate. But in CapCut that warmth sits UNDER blush, lipstick and lashes, which give the face colour variety. With the makeup absent the identical cast lands flat on an unpainted face and reads as pink skin instead of glow. **So the number that matches the measurement is not the number to ship** - the measurement was never wrong, it was answering a question with the makeup still in it.
**AND THEN THE DIAL WAS THE WRONG ANSWER, which is the part worth keeping.** She put the warmth-0.25 frame beside hers: it had gone grey and flat, measured 5.8 red and 3 luma short of her export. Turning warmth DOWN was treating the symptom. **The warmth was never the fault - the missing thing was GLOW.** Her export runs 19.45% near-white against the pipeline's 17.89%: highlights that spread and bloom. Bloom is what turns warmth into glow instead of into pink skin, so WITH it the full measured warmth reads correctly, and WITHOUT it no warmth setting is right - too pink at 1.0, grey at 0.25. Warmth went back to the measured 1.0 and v3 matches her export to R -1.1, G -0.8, B +0.5, luma -0.48.
...
**Bloom was rejected once before and this is not a reopening of that.** The note in `finish-video.mjs` says it was tried stacked with vignette and grain and that "each was subtle on its own; stacked, they turned a clean image muddy". This is bloom alone at 0.09. If she calls it hazy, that is settled and it comes out.
**Warmth is now a dial, not a constant.** 1.0 is the measured truth, 0 keeps the identical tone curve with no colour separation at all, brightness unchanged at every setting. One direction at several strengths. Her pick is pending against `FILTER-TEST-warmth-ladder.png`.
**Evidence.** `~/Movies/riadms-work/WATCH-THESE/FILTER-TEST-v2.mp4` (corrected) and `FILTER-TEST-warmth-ladder.png` — 30s, her CapCut export on the right, the pipeline plus the measured filter on the left. Colour is indistinguishable; the missing makeup is plainly visible in her lips and cheeks.
**Status.** live

### 2026-09-18  put the word series inside the reel that works, and find the limit is footage
**Decision.** Build **words you should know but don't**, the he10 structure carrying the "there's a word for it" idea: her dancing, a title card, then a word, what it means and a small picture on the same 2.73 second beat. Queue it second in the reel lane, directly between the books version and the text-short version of the same idea. Her ask: *"kind of like how we started the red flags. And then we put a word and what it means and then a little image that would make sense."*
...
**What changed.** `scripts/word-reel.mjs`, `instagram/words/should-know.json`, five stills cropped out of her own b-roll into `instagram/words/images/`, and a section in `text-shorts/WORD-FOR-IT.md`. The five words are limerence, demisexual, fawning, sonder and the spotlight effect, all real and checkable, none of them anywhere in the queue.
...
was protecting nothing: everything inside the gap is her own continuous take, so
loosening can only restore sound she actually made, and every candidate already has to
pass `confirmed(strict=True)`, which reports a restored stumble as scrap and refuses it.
The test was the safety net all along. Tightening REMOVES her sound and has no
...
### 2026-09-18  Friday and Sunday filled from exports she had already made
**Decision.** Move `a little life` (11:35) from Wed 23 Sep to Fri 18 Sep, and give the
Sunday 20 Sep slot to her 8:36 alterous export instead of the 8:06 cut that held it.
**Why.** She asked for a video on Friday and Sunday. Friday was empty, and Sunday held
the OLD 8:06 alterous, which still says "not as much as romance card" - the fault she
caught herself and which later renders fixed. Re-cutting either one for this weekend was
not possible, because a re-cut needs her CapCut pass and Friday was under twelve hours
away. Both slots were therefore filled with files she had already exported, which cost
her nothing.
...
**Why.** All three had a finished CapCut export on her Desktop, a render on disk, and
nothing on the channel pointing at them. All three are in the 12-18 minute band, which
delivers 4.64 watched minutes per view against 3.61 for 8-12, and which only five of her
...
three has a thumbnail either. Both need her.
### 2026-09-18  the six new essays are written, and the lane is blocked on her rather than on writing
**Decision.** Write all six pieces the ladder assigned, plus the NYT Opinion re-cut, and stop calling the lane blocked on drafting.
**Why.** Her ask for the session, in her words: *"I want you to like finish literally everything that is related to essays and write everything and like make a strategy for us to get published."* The finding underneath the work is that not one of the five outlets that can still produce a November credential was waiting on prose. Four of them are sitting in Gmail or on a form, waiting on about four and a half hours of her, and the biggest, Autostraddle, has been overdue since 26 August.
**What changed.** Seven new files in `out/press/`: `sun-roots.md` (683 words), `aze-social-expectations.md` (1,186), `narratively-the-song.md` (1,006), `rumpus-sor-juana.md` (1,588), `business-insider-essay.md` (946), `slate-essay.md` (1,109), `nyt-opinion-six-words.md` (1,002). Every one scores 0 to 2 of 100 on `ai-tells.mjs` and classifies HUMAN_ONLY, and every one was measured against her essay register on `her-voice.mjs` and rewritten where the contraction rate or the question rate was outside her band. Nine slots are marked across five of them, and every slot is a true thing only she has.
**Decided and deliberately NOT acted on.** Tiny Love Stories, which the strategy names as the best value per hour in the whole folder and which is not drafted. It is 100 words of her own life plus a photograph, and a hundred-word micro memoir assembled by somebody else is exactly the thing that reads as assembled.
**Evidence.** `out/press/PUBLICATION-STRATEGY.md`, and the per-file Notes sections, which each carry a sourced-versus-written table.
**Status.** live
...
**Status.** live, and it is a decision waiting on her

### 2026-09-18  three live pieces stand on the six Greek words, and one of them has to give
**Decision.** Record the collision and put the choice in front of her rather than resolving it unilaterally. The proposal is that the Greek-words section comes out of the Autostraddle essay before its form is filled in.
...
**Status.** live, waiting on her before Autostraddle is submitted
...
**Why.** Her instruction: *"when you're posting about journaling, can you add as a contributor Nuuko app, my journaling app, @nuukoapp"*. A collaborator is not a mention: the post appears on BOTH grids with both names on it, so the love-journal lane feeds the app's account as well as hers, at no cost per post. Nuuko is the capstone product and has no distribution of its own; the journaling lane already posts daily at 22:45.
...
**Why.** Her words, on being told types-of-jealousy (5:11) and am-i-anxious (6:10) could not reach 8:30 honestly: *"it's fine it doesn't matter the length as long as they are perfectly edited"*. The floor decides what to film; a take already filmed short has nothing true to put back.
...
**Decision.** Her 33 CapCut re-exports of tonight replace the scheduled cuts in place: each new
...
are uploaded; the rest follow as she finishes.
**Why.** Her words: *"these are like the videos that we recut for quality"*, and *"tomorrow morning
...
its job table moved out to JSON, because she now re-cuts in batches and a new batch should not mean
...
**Decision.** Her new export (`a little fl.mov`, 11:34) is cleared for its slot. The one genuine
...
export ("Judy" for Jude, "possible" for possibility, "further" for "far away", which is hers
...
relative pronoun. That is one missing function word in a fast aside, and asking her to redo a CapCut
...
### 2026-09-15  seven-types-of-love is dropped from the batch because it is already public, and the fault found in the take never reached it
**Decision.** `seven-types-of-love-sternberg` is **removed from the render queue**. It is already on YouTube as "7 types of love, and the one that lasts 40 years", public since 2026-06-27, 5,162 views. The three remaining long-forms are bridgerton, romance-books and procrastination.
**Why.** `edit-preflight` reported ALREADY ON YOUTUBE (01bbxGmkpPQ). This should have been checked before the take was prepared, not after: it is a standing rule that most raw takes are already published, and the cost of skipping it here was one full preparation pass.
**The thing worth knowing, and it is good news.** The paragraph-read found that this TAKE states Sternberg's fourth type wrong: she says "romantic love, intimacy plus commitment" and then describes companionate love for ninety seconds before catching herself. If the published video carried that, it would be a live error teaching a wrong pairing. It does not. The published description's own chapter list reads "3:02 romantic love: intimacy and passion without commitment" and "3:33 companionate love: intimacy and commitment without the fire", both correct, so whoever cut it in June removed the failed attempt. **No public correction is needed.**
**What changed.** Removed from `_state/queue.txt`. The work is not wasted: three name patches are now in its words.json for any future re-cut (Sternberg, said wrong at 57s and right at 840s; Gottman for "kottman"; limerence for "Lumerance"), and the decisions file documents the fourth-type fault and the two deliberate non-cuts.
**Evidence.** `mcp yt_get_video 01bbxGmkpPQ`; `assets/decisions/seven-types-of-love-sternberg.decisions.json`; `out/passes/seven-types-of-love-sternberg.paragraph-read.md`.
**Status.** live.
...
**Why.** Splitting the over-60s videos by length: 61-75s converts at 3.08 subs per 1k, **76-90s at 5.64 and holds 56% viewed**, 91-120s at 3.94, 121-180s collapses to 0.92 and 49%. The subs column is thin, five and six videos a band, and 1,922 of the 2,030 in the best band is one video, so the load is carried by **% viewed**, which does not depend on the outlier and orders the bands the same way. The five terms were chosen off her own comments rather than off a keyword tool: 3,502 likes on an aroace viewer wanting to share a life with their best friend, 416 on someone who could know their crush for years and still not know, 381 on someone who thought it was a crush and did not want a relationship.
**What changed.** The scripts file. `CLAUDE.md` rule 17 and `RPM-AND-REVENUE.md` already re-worded earlier today.
**What was verified.** Checked against her own voice, not only the AI list: burstiness 1.26 against the 0.60 floor, specificity 67.2 against her corpus 25.3, em dashes 0, `ai-tells` 2/100 HUMAN_ONLY. Limerence was checked against the live schedule and dropped from the batch because "how to know it's limerence, not love, in 141 seconds" already publishes on 27 October.
**What was caught in review.** The first draft used em dashes in five section headings, which is her absolute ban, and carried a claim that a 416-like comment was the most liked thing on her channel. It is not; a 3,502 comment is in the same file. The claim was cut rather than softened.
**Status.** live

### 2026-09-15  a mini cannot be lifted out of an essay, and her question caught it before anything shipped
...
**Why.** She asked, *"minis are not shorts they need thumbnails and everythign r u sure they are good enogh to make them a single video?"* and `repeat-scan.mjs` answered it. All three BLOCKED on real faults: mini 1 says "want to be in a relationship with" twice 4.5s apart; mini 3 says "like a lot of people" twice 5.2s apart and "it doesn't have to be" twice 1.1s apart; mini 2 restates its own point at 76s. The cause is structural. Inside an 11 minute essay a phrase recurring five seconds later is invisible; inside a 107 second lift it is the whole video, so **a lift inherits the parent's repetition at a length where it cannot hide.** A second gate can never clear at all: joins are verified against the raw take, and a lift cut from the published file has no take, so NO-TAKE blocks it permanently. And her packaging point stands on its own: a mini is a video on the channel and needs a thumbnail and a title, and none of the three had one because I was treating them as feeders.
...
**Decision.** The sub-minute lane is retired and the **mini** replaces it: 75 to 150 seconds, wide, lifted out of a published essay. `CLAUDE.md` rule 17 and `RPM-AND-REVENUE.md` are re-worded to say so, on her instruction to do it myself. Three minis are cut and none is scheduled.
**Why.** Her correction, *"any short above 1 minute is actually a long form pls undertand that"*, re-cut the measurement at the minute: 233 videos at or under 60 seconds gave **37 net subscribers** between them, 22 over 60 seconds gave **2,188**. A cliff, not a slope, since 46-60s converts at 0.58 with the six second cards while 61-90s converts at 5.57. The 4.3-5.5 subs per 1k that rule 17 used to defend Shorts, reels and updates was always a measurement of the 61-180 second wide videos; the sub-minute ones convert at 0.10.
**What changed.** `scripts/mini-cut.mjs`: lifts a passage out of the file YouTube actually serves, using the published captions' word timings, wide, no vertical crop. It has a hard floor at 70 seconds and refuses a window that crosses one of her own section openers, which is what stops a mini being two arguments. `CLAUDE.md` rule 17 and `RPM-AND-REVENUE.md` §4 re-worded. `out/minis/*.picks.json` holds 30 selected passages across 9 essays.
**What was verified rather than assumed.** That her biggest one is wide was read off its own frame sheet, not taken from the research note. The download was checked against the published duration (660.4s against 661s) before anything was cut from it, because neither the pipeline render (1224s) nor anything on the Desktop matches these essays. All three renders were looked at as contact sheets rather than trusted from the log.
**What is blocked and was not worked around.** `repeat-scan.mjs` is refused by this session's permission classifier, and rule 4 says nothing gets a publish date until repeat-scan has read the file. So the three minis are rendered and **unscheduled**. The gate did its job; it is hers to run or to permit.
**What was thrown away.** A first VTT parser read every word twice, because YouTube's rolling caption format repeats the previous line above the new one and taking the whole cue body glues the repeat onto the first new word. A first section detector caught "the fourth type" and missed "the first one" and "the type two is", so the first mini it produced ran through two of her list items.
...
**Decision.** The cut engine stops trusting whisper's word boundaries for anything about air, and every long-form and mini is re-cut and re-delivered into `WATCH-THESE/finished needs capcut`. Her prompt: *"there's like still a moment around 0:50 where it's like just me breathing in the mic"*, and then *"make sure these never happen again"*.
**Why.** She had reported the same thing once before, on 30 August, and it was answered by lowering `beatKeep` to 0.25, which reduced the symptom and never touched the mechanism. Measured on the raw take this time: she finishes "the magnitude of the threat" at 172.96, the room is silent to 173.7, she draws breath from 173.8 to 175.0, silent again to 175.9, and says "If your partner" at 176.0. A breath averages well under the -30dB line but peaks around -14, and `silencedetect` needs the level to stay under for the whole span, so it reports the two silences and not the breath between them. Four separate rules then did the right thing with that wrong input: the island weld gave the pause back because it asked what CLASS of cut made the gap rather than what was in it; whisper had put "if" three seconds early and a second long, so the silence and the breath sat inside a word span that the pause rules may not trim and that `touchesAWord` correctly held; the gap was trimmed as two silences rather than one piece of air, so a short one failed `--mid-min-save` and was left alone; and a word-removal cut started at whisper's start for the word it removes, leaving the front of it on the kept side of the join, which is the second fault she reported the same morning on four-conversations.
...
**Why.** She asked why the drive holds 1.8 TB and assumed duplicates. It does not: a full inventory of all 9,791 files over 10 MB found only **0.5 GB** of genuine duplicate files. The drive is large because she films 30-60 minutes of 2.7K HEVC at 31 Mbps per video and keeps all of it, 439 takes. The ugly-b-roll cleanup she asked for is real editorial work but the entire b-roll folder is 4.1 GB of 1,800, so it could never have been the answer. Re-encoding beats deleting because the footage stays re-cuttable: measured SSIM against source is 0.9932 at 14 Mbps, and a frame-exact comparison of her hair edge against the pale wall is indistinguishable.
...
**Decision.** After she asked that these mistakes not happen again, the read is enforced in code and carried in files rather than remembered: no take-bound long-form gets a publish date without a dated `_read_as_paragraph` note in its decisions file; a `paragraph-read` agent carries the method; CLAUDE.md rule 4, SHIP-CHECKLIST section 6 and the edit-video skill require it; and the cutter's restart rule is being tested for a guard against cutting her lists.
**Why.** The same read over 16 long-forms found about 110 faults that every scan and a full listen had passed, in two directions: stumbles left in, and her own lists and parallels cut as retakes. A note in LEARNINGS does not stop the next video; a refusal at the publish date does, because every video passes through it.
...
**Why.** Full listens of the morning renders found friendship saying "to understand what of it" where she said "what love is", because a hand cut from the day before started mid-word, and underrated keeping her sung "la la la la la", which the transcript had hidden inside "because". types-of-jealousy was refused by the doubling gate over a digit.
...
**Decision.** Named as the account's most valuable unknown rather than explained, and the insights scope put in front of her as the fix. No theory was written into the brief.
**Why.** Likes 4,910 against 4,856, comments 239 against 238, output 51 posts against 46, and this week's posts are seven days younger, so at equal age engagement is up. Follows went from +73 to +39 anyway. Every candidate explanation sits between reach and following, and reach, views, saves and shares are not on the Instagram token. It also weakens the first brief's "followers = breakouts x about 20" model: 4 breakouts gave 18 each, 3 gave 13. Writing a cause from likes alone would have been inventing a number, which the job forbids.
**What changed.** Nothing in the pipeline. The insights scope is item 3 of `scripts/.ig-brief-weekly-report.md` under "what needs her", and it matters from tomorrow because love-journal is a saves lane that will read as a failure on likes.
...
**Why.** Her words: cancel whatever goes out tomorrow "because I don't think we'll be ready", and
...
no clip stay, and the 13 "rough cut.srt" files (hers, not DJI's) stay. The 13 videos that existed
...
### 2026-09-13  renders stop when the drive is nearly full, and the batch waits for her
...
freeing space on her library drive without asking.
...
drive, which is her call: the last cleanup was approved group by group.
...
riadms-work/_rescue-2026-09-13). Options measured for her: superseded renders 8.6 GB, cache/work
...
treated as done; a file the verifier marked BLOCKED is not handed to her.
...
the file sat in WATCH-THESE, and she watched it and caught the line herself. The cause was an August
decisions entry that cut only the first of her three tries at that clause, so the cutter kept half of
...
pause (option B of three she was shown as numbers).
**Why.** She watched the first re-cut and said it still cut often and cut names. The morning's fix
...
### 2026-09-13  a publish date also needs every sentence to be one she actually said
**Decision.** The publish gate now blocks on SEAM, a cut that welds two attempts at a sentence into
one she never said, and on NO-TAKE, a long-form scanned without its raw take to compare against.
...
discarding short pieces of real speech next to her pauses; cut-video.mjs was fixed to keep any piece
...
**Decision.** No video gets a publish date until scripts/repeat-scan.mjs has read the file she
...
flags were her phrasing on purpose, which are ruled in writing rather than skipped.
...
**Why.** It had not vanished. She sent it on the 12th at 19:33Z, along with the Yasmin
...
re-created for no reason, and had she not already sent it, two near-identical emails
...
### 2026-09-12  every audit finding is fixed on main, and the deploy waits for her
**Decision.** Her instruction, "fix all of them": every row of /nuuko-audit was fixed on `fix/audit-2026-09-12` and merged to main in nuuko-v1 (`0efea21`). Analytics loads Vercel's own insights script on all 32 pages. The streak popup, the dead `getStreakData` call, the day-streak number and the count badges on save are gone. No mood guess while typing, and the mood question now reads "now it is written, how did it feel?". Home opens on the moment, then the people, then the greeting. The library filters by person. Onboarding is three cards. The sign-in banner waits for the third page. A new person's first prompt is always a noticing one, and her 200 questions display lowercase. On a phone: the landing button is inside the first screen with a sticky button after scrolling, the home tab icon is visible, settings inputs are 16px, and onboarding tap targets are 44px.
...
**Not acted on.** Production deploy. The CLI deploy was refused by the permission check, so www.nuuko.app still serves the earlier build until she runs it or allows it. The settings toggles are left under 44px.
...
**Why.** She said the strategy numbers already exist and the craft is what is behind. A viewer,
@alipercapita, commented on "being asexual taught me why people pick the wrong partner" that at
about minute 6 she says the same sentence twice, and the next one twice too. Transcribing the file
she actually uploaded confirmed it at 5:59-6:11. That video rendered at 13:39 on 7 September, and
...
being them" is her parallel on purpose, and a gate that is wrong twice in three gets overridden.
...
### 2026-09-12  a reply answers only what is open, and she showed the length by sending one
**Decision.** A reply inside a thread is capped at **100 words and never longer than the email it
answers**, and it answers only the items actually open. Supersedes the 150-word ceiling set
earlier the same day.
**Why.** After the 150 ceiling I cut the North Atlantic reply to 148 words. She sent her own at
**72**, against Julia's 105. The difference was not sentence length. She deleted four topics: the
addresses Julia already had and had confirmed, "please don't chase Sherronda" when nobody offered,
the printed-copies-only condition (a new ask raised in a reply), and the riadms.com/shelf promise
already made in the cold email. My version shortened each of those and kept all of them.
**What changed.** `.claude/skills/draft-email/SKILL.md` section rewritten around her sent email as
the reference, with the four deletions as rules. `scripts/email-html.mjs --reply` now 100, plus
`--theirs <words>`. The Yasmin Benoit reply re-cut again, 148 to 62 words against her 62.
**Evidence.** Her sent message on thread `1a07cd483b4e99e0`. Her version passes the checker at 72;
the 148-word draft now fails.
**Status.** live. Supersedes "emails get a word ceiling" (2026-09-12) on the reply number.

### 2026-09-12  nuuko is on main and actually deployed, and the git trigger is confirmed dead
**Decision.** Her instruction, twice: push nuuko to origin main. Everything from the
...
**Still hers.** Migration `006_circles.sql` is not applied, so the circle cannot be
...
**Decision.** The second live reply in her drafts was rewritten to the 150-word ceiling: 330 words
...
for asking her, and a verbatim restatement of the question she had already quoted back in her own
...
### 2026-09-12  emails get a word ceiling, because "not soundin smart" is not an adjective
**Decision.** A reply inside a live thread is capped at **150 words of prose** and a cold email at
**300**, checked mechanically by `scripts/email-html.mjs --check --reply|--cold`, which exits
non-zero over the line. Address blocks and the signature do not count. Over the ceiling, the
instruction is to cut a whole thing, never to reword.
**Why.** Her correction: *"the replies that we give are so so hard to read and long they should be
shorter and more like easy to read and straight to the point not soundin smart."* The North
Atlantic Books reply she was reading ran **241 words against the 105-word email it answered**, at
14.2 words a sentence, with four sentences over twenty words. The register rules from 2026-09-03
(one idea per sentence, contractions, plain words) were already written and the draft passed every
one of them. Nothing measured length, so the emails stayed correct and stayed long.
**What changed.** `scripts/email-html.mjs` now measures the prose on every `--check`: words,
sentences, words per sentence, contraction rate, and each sentence over twenty words quoted back.
It also gained a `---sig:short---` mark, because the builder could only emit the full signature
block and that is why a fourth-message reply was carrying a cold-email footer. New sections in
`.claude/skills/draft-email/SKILL.md` and `.claude/skills/email-format/SKILL.md`. The Julia draft
was rewritten to 149 words at 9.3 a sentence and replaced in her drafts.
**Evidence.** `node scripts/email-html.mjs <draft> --check --reply`. The Gmail draft on thread
`1a07cd483b4e99e0`. The four cuts that took 92 words out are listed in the skill.
**Status.** superseded the same day on the reply ceiling: 150 became 100, see the entry above, because
she sent her own reply at 72 words.
...
**What changed.** The College LOs are now real hashtags read off her Outcome Index rather
...
folder is still not organised, and the Cover Sheet itself cannot be copied without her, so
...
### 2026-09-11  the quality pass: the pivot on every screen, and the streaks finally gone
**Decision.** Every user-facing page of Nuuko was loaded at phone and desktop width
with seeded people and read as screenshots, and what contradicted the pivot was fixed
on the branch `capstone/quality-pass` in `nuuko-v1`, on top of the context branch.
**Why.** Her ask: go through everything, improve the design and the experience, fix
the problems, make it feel like an app at the quality of the ones on the market, with
the person twist. What the screenshots showed was that the pivot had landed in the
data model and the shelf and nowhere else. The home hero still showed a "3 day
streak" pill, patterns still had a day-streak stat, settings had a current-streak
stat, and the profile page was streaks, badges and a "journaler identity" and threw a
JS error on load. Login said "continue your journaling journey". Settings sold a
referral programme with "30 days of premium", a coming-soon card with an email
capture and a "journaling commitment". The landing said "Cozy Journaling App" in the
title and had a section for badges, archetypes and challenges. Onboarding said "your
feelings, your space" and ended on "start journaling". A person's handle was forced
lowercase by CSS, which breaks the one exception to the lowercase rule. And on a
phone the notebook was 600px tall, so the who-is-this-about picker, the one thing
that makes the app different, sat a full screen below the fold.
**What changed.** Streaks removed from home, patterns, settings; profile retired to a
redirect. People shelf on home above the months. Patterns opens with "who it was
about" (pages, since, last written) and its streak stat is a people count. The person
page shows "the first thing you wrote about them" once there are two pages. The
onboarding's four slides are rewritten to the pivot and the last one takes a handle,
puts that person on the shelf and opens their first page. Landing hero, title, meta,
buttons and closing line rewritten; the badges section removed; the waitlist moved to
the end. Write page titles "page N" instead of "first entry" on every page (a storage
race), the phone notebook starts at 300px and grows. Two dead script tags, a stray
"people" label, the 2025 footer, a per-session sign-in banner, and "0 words" on
imported pages are fixed. The full list is `nuuko-v1/about-nuuko/quality-pass.md`.
**Not done, on purpose.** `challenges.html` (30-day challenges, gratitude, badges) is
pre-pivot and unreachable except from a hidden settings section; left for a later
pass. The mood cards on patterns stay, demoted under the people card. The landing
below the hero still says journaling in the testimonials and the founder story.
**Evidence.** `node scripts/smoke.cjs` in `nuuko-v1` exits 0 with zero page errors
after every batch; screenshots at 390px and 1280px, light and dark, were read. Branch
`capstone/quality-pass`, commits `57c6f4c` and the one after it. Main is protected,
so the merge and the preview test are hers.
**Status.** live, awaiting her test of the preview
...
### 2026-09-11  nuuko.app had not deployed since 1 April, and the person pivot is live now
**Decision.** Fix the build and deploy production from the CLI rather than wait for
the GitHub integration, then verify on www.nuuko.app/people rather than on the push.
**Why.** Every push to `nuuko-v1` from 11f94cf on 2026-04-01 to today failed
`npm install`: `@capacitor/core` pinned at ^7 while every `@capacitor/*` plugin
asked for >=8. Vercel kept serving the 9 March deploy behind a green domain, so
the site looked fine for five months and nobody knew. The three pushes on
2026-09-11 (the pivot, the skills, the merge) produced no deployment at all on
either Vercel project, and the commit status on GitHub sat at pending. The desk
said "live on the next deploy", which was a promise nobody could keep.
**What changed.** `nuuko-v1` package.json: all Capacitor packages on ^8, `engines.node`
22.x to stop the auto-upgrade to Node 24, and `package-lock.json` committed and
un-ignored so the tree cannot drift again. `npx vercel link --project nuuko` then a
preview deploy, checked behind the share cookie (/people 200, login reaches Supabase
and returns "invalid email or password", not a 500), then `vercel deploy --prod`.
www.nuuko.app/people returns 200 and the person shelf is on it. Two Vercel projects
(`nuuko`, which owns www.nuuko.app, and `nuuko-v1`, a duplicate) are linked to the
same repo and both build every push; the duplicate is left alone tonight and is a
candidate for deletion so the 100-a-day cap is not spent twice.
**Evidence.** Vercel build log for dpl_Q7FqoCkSykiWmobywrTgDrpt58Mc (ERESOLVE),
`nuuko-v1` commit cb11dd6, `curl -I https://www.nuuko.app/people`.
**Status.** live. Open question: why the GitHub app produced no deployment for
today's pushes even after the fix. Watch the next push; if it does not deploy on its
own, the CLI is the route until someone reconnects the GitHub app in Vercel.
...
**Status.** live as a finding; the migration waits on her hand (see next entry).
...
### 2026-09-11  the person pivot is built, on a branch, and tested in a real browser
**Decision.** `nuuko-v1` branch `capstone/person-filing` carries the pivot: a
`people` store in storage.js with per-person delete that keeps the pages, an
entry field `people[]`, the picker on the write page after the editor and before
the mood, `/people` as a shelf of person books and a person page with the seven
kinds bars, the handoff from riadms.com/which-one-is-this that turns a result
into a person and their first page, the prompt engine routed through her 200
questions with the gratitude prompts gone, the character reacting to the return
instead of the streak and noticing the person the user usually writes about at
this hour, and export carrying people. Nothing pushed to main.
**Why.** Every item is a decision already logged today, and the strategy said
the pivot changes the data model so everything else waits behind it. A branch
because she tests before it goes live, and because a wrong data model on main
is the expensive mistake.
**What changed.** 14 files in nuuko-v1 plus `scripts/smoke.cjs`, three skills,
two agents and a rewritten CLAUDE.md there. In riadms, the keep-it block on the
diagnostic result page. `capstone/LANES.md` says which chat does what next.
**Evidence.** `node scripts/smoke.cjs` in nuuko-v1 loads six pages, files an
entry under M, finds M on the shelf with one page, accepts a handoff for J with
two kinds bars, and exports two people, with zero page errors. Screenshots read.
**Status.** live. Merged to main on her word the same day, so nuuko.app deploys it next.
...
notices. Her measured audience is 16.8% aged 13 to 17, which is also the COPPA
exposure that cost Path $800,000 and twenty years of assessments. Under the DSA she
...
**Status.** live, and it is hers to overrule.
...
titles.
**Why.** The word journal appears zero times in 745 of her YouTube comments. So do
...
**Evidence.** `capstone/research/04-her-audience.md`.
**Status.** live.
...
### 2026-09-11  the prompts are replaced with her own 200 questions
...
exactly what her own competitive analysis says makes the category interchangeable.
Meanwhile 200 questions written to her voice about specific people already exist in
...
minutes, 72% of all attention. And her six journaling long forms get 312 to 936 views
yet convert at 8.65 likes per 100 against her 4.21 average, ranking first of 32 on
subscribers per thousand. The smallest videos convert best.
**Evidence.** `research/07-the-channel-numbers.md`, `research/04-her-audience.md`.
**Status.** live.
...
prompt corpus.
**Why.** Her ask was whether crush and relationship journaling is working. Measured on
...
`journal with me` lane is stationery creators at 3,000 to 40,000, which is the lane her
...
Her own six journaling videos already convert best on the channel and reach worst, so
...
**Status.** live. Extends 2026-09-04, which reached the same conclusion from her own
numbers alone.

### 2026-09-11  the series name and the Nuuko hand-off are the two things blocking it
**Decision.** Nothing in the series gets filmed until she picks a series name, and every
...
documentation.
**Why.** Her call, agreed with her advisor. The two halves are causally connected and
...
wrong.
**Why.** Her own correction, said out loud. The ledger and six resumes carried the
larger number, and it is the one an interviewer or a second reader asks her to break
down.
...
**Why.** 63 per cent of her views were on videos too short to carry a mid-roll, five
...
**Status.** **superseded 2026-09-11.** She and her advisor merged them. The risk that
...
**Decision.** On reels and Shorts, text may repeat what she says, the grade is a
variable, the music is open. She picked Playfair Display by looking at renders over
her own footage.
**Why.** The essay rules were written for a ten-minute horizontal video and forbid the
exact devices the reels she wants to imitate are built from.
